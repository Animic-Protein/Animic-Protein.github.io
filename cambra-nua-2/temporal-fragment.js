...
export function createTemporalFragment(detail={}){
 ...
 record=evolveRecord(record,'relation',{kind:'born-in-organ',target:'cambra-nua-del-temps',suggested:false,canonical:false,reversible:true,traceRef:record.provenance.originId});
...
export function relateTemporalFragment(record,destinationKey){
 ...
 const existing=(record.relation||[]).find(rel=>rel.kind===dest.kind&&rel.target===dest.target);if(existing)return{record,relation:existing,deduplicated:true,destination:dest};
 const next=evolveRecord(record,'relation',{kind:dest.kind,target:dest.target,label:dest.label,href:dest.href,traceRef:record.provenance?.originId||record.source?.id,suggested:false,decisionRequired:true,canonical:false,reversible:true});
 const registry=load();const key=Object.keys(registry).find(k=>registry[k]?.provenance?.rootRecordId===record.provenance?.rootRecordId||registry[k]?.source?.id===record.source?.id);if(key){registry[key]=next;save(registry)}return{record:next,relation:next.relation.at(-1),deduplicated:false,destination:dest};
}
