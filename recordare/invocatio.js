(()=>{
  ...
  function mount(){
    ...
    panel=document.createElement('div');panel.className='recordare-panel';panel.hidden=true;panel.innerHTML='<div class="recordare-shell"><button class="recordare-close" aria-label="Tancar RECORDARE">×</button><iframe title="RECORDARE · Invocatio" allow="microphone"></iframe></div>';
    document.body.append(button,panel);
    button.onclick=open;
    panel.querySelector('.recordare-close').onclick=close;
  ...
  function receive(e){if(e.origin!==location.origin||e.data?.type!=='codex:recordare-captured')return;lastCapture={...e.data.detail,returnedTo:location.pathname,receivedAt:new Date().toISOString()};window.dispatchEvent(new CustomEvent('codex:recordare-returned',{detail:lastCapture}));}
  window.RecordareInvocatio={open,close,lastCapture:()=>lastCapture,context};
