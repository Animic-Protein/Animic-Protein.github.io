(() => {
  ...
  function ensure(){
    ...
    box=document.createElement('aside');box.id='impulsPanel';box.setAttribute('aria-live','polite');box.style.cssText='margin:2rem auto 0;width:min(560px,100%);padding:1.15rem;border:1px solid var(--o);border-radius:18px;background:radial-gradient(circle at 10% 0,#17384a,#06131f 65%);text-align:left;box-shadow:0 0 36px #00c8ff18';stage.insertBefore(box,stage.firstChild);return box;
  }
  function render(){
    const box=ensure();if(!box)return;const d=suggest();
    box.innerHTML=`<p class="ey" style="margin:0 0 .35rem">IMPULS · ${d.state}</p><p style="margin:.2rem 0;color:var(--t);font:1.45rem Georgia,serif"><strong>${d.label}</strong></p><p style="margin:.35rem 0;color:var(--m)">${d.text}</p>${d.href==='#'?'':`<a class="go" href="${d.href}" style="margin-top:.6rem">Seguir aquest gest</a>`}<p style="margin:.7rem 0 0;color:#78909c;font-size:.75rem">Proposta del Còdex · reversible · no canònica · tu decideixes</p>`;
  ...
  window.CodexImpulse={suggest,render,expose};
})();