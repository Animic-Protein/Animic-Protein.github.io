...
El `generated-phenomenon` torna com a presència no canònica i reversible. El receptor només suggereix una ruta perceptiva i espera decisió humana.
...
IMPULS pot suggerir el retorn d'un fragment temporal, però no el decideix. La Formiga tampoc apareix pel simple fet d'haver generat un fenomen.

Ruta mínima:

```text
generació
  → ruta perceptiva suggerida
  → decisió humana
     ├─ observar → Rosa / Cambra
     │              └─ possible fragment temporal → circulació → nova decisió humana
     ├─ conservar → RECORDARE
     └─ deixar quiet
```
